...
// Diccionario de Países y Ciudades (con sus códigos IATA para el backend)
...
export default function AgregarVuelo({ userInfo, setActiveItem }: { userInfo: any, setActiveItem: (item: string) => void }) {
  ...
  // Manejadores para resetear la ciudad cuando se cambia el país
  ...
  // Restricción estricta para que nunca pase de 60 asientos
  ...
  // Función para extraer solo las letras del IATA (ej: de "París (CDG)" saca "CDG")
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl space-y-8">
        ...
        {/* SECCIÓN DE DETALLES TÉCNICOS Y COMERCIALES */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-semibold text-sm text-slate-700">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input type="number" min="1" step="0.01" value={precio} onChange={(e) => setPrecio(e.target.value)} placeholder="Ej: 12500" className="w-full p-4 pl-12 bg-slate-50 border border-slate-200 rounded-xl outline-none font-black" required />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Máx. 60"
                className="w-full p-4 pl-12 bg-slate-50 border border-slate-200 rounded-xl outline-none font-black text-green-700"
                required
              />
            </div>
            <p className="text-[10px] font-bold text-slate-400 text-right mt-1">El límite máximo permitido es de 60 lugares.</p>
          </div>
        </div>
