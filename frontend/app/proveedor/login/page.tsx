...
import { Building2, ArrowLeft, Lock, Mail, AlertCircle, Eye, EyeOff, Loader2 } from 'lucide-react';
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-10">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 mt-4 active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Iniciar Sesión de Socio"}
            </button>
          </form>
