...
import { authService } from '@/lib/authService'; // Importamos la lógica centralizada
...
export default function ClienteRecuperarPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <Link href="/cliente/login" className="absolute top-8 left-8 flex items-center gap-2 text-slate-500 hover:text-orange-500 transition-colors font-bold text-sm">
      ...
      <div className="w-full max-w-[450px]">
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-10">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 mt-2 font-medium text-sm">Ingresa tu correo y te enviaremos las instrucciones.</p>
          </div>
          ...
          {status === 'success' ? (
            <div className="bg-green-50 border border-green-200 p-6 rounded-[2rem] text-center animate-in zoom-in duration-300">
              ...
              <p className="text-green-700 text-sm font-medium">Revisa tu bandeja de entrada para continuar con el restablecimiento.</p>
              <Link href="/cliente/login">
                <button className="mt-6 w-full bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition-all shadow-lg text-sm">
                  Volver al Login
                </button>
              </Link>
            </div>
            ...
            <form onSubmit={handleRecuperar} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-black uppercase tracking-wider text-slate-400 ml-1">Correo Electrónico</label>
